...
/**
 * @file
 * Contains \Drupal\walk\Form\DayFourWalkForm.
 */
...
class DayFourWalkForm extends FormBase {
  ...
  $walker_image =$node->get('field_day4_pic')->getValue();
  if(!empty($walker_image[0])){
    ...
    $walker_dist =$node->get('field_day4_distance')->getValue()[0]['value'];
  }
  else{
     $walker_image = 0;
   ...
   */
    ...
    /* $user = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
      ...
      if (($form_state->getValue('day4_walk_distance')) > $event_type) {
        $form_state->setErrorByName('day4_walk_distance', $this->t('Your distance is greater than the total event type.'));
      }*/
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    $image = $form_state->getValue('day4_image');
    $distanace = $form_state->getValue('day4_walk_distance');
    ...
    foreach ($nids as $nid) {
      ...
      $node->field_day4_pic->target_id =$image[0];
      $node->field_day4_distance->value =$distanace;
  ...
  $current_walk_distance= $distanace+$walker_day1_dist+$walker_day2_dist+$walker_day3_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;

  $pending_walk =$walker_total_distance-$distanace-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day5_dist-$walker_day6_dist-$walker_day7_dist-$walker_day8_dist-$walker_day9_dist-$walker_day10_dist;
 ...
 $overall_walk =$walker_day1_dist+$distanace+$walker_day3_dist+$walker_day2_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
...
if($walker_total_distance == $distanace || $pending_walk == 0 ){
  ...
  $certificate_html = getHtml4($walker_full_name, $walker_total_distance, $event_name, $days);
...
function getHtml4($walker_full_name, $walker_total_distance, $event_name, $days){
    $my_html='<HTML>
