...
/**
 * @file
 * Contains \Drupal\walk\Form\DayFiveWalkForm.
 */
...
use Drupal\Core\Url;
class DayFiveWalkForm extends FormBase {
  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'day_five_form';
  }
  ...
  $walker_image =$node->get('field_day5_pic')->getValue();
  if(!empty($walker_image[0])){
    ...
    $walker_dist =$node->get('field_day5_distance')->getValue()[0]['value'];
  }
  else{
    $walker_image = 0;
  }
  ...
  'min' => '0',
    ...
    if(!empty($walker_image_url)){
       $form["step_5"] = array(
        ...
        $form["step_5"]['day5_walk_distance'] = array (
      '#type' => 'textfield',
  ...
  'min' => '0',
    ...
    $form['#cache'] = ['max-age' => 0];
    $form['step_5']['actions']['#type'] = 'actions';
    $form['step_5']['actions']['submit'] = array(
      '#type' => 'submit',
      '#value' => $this->t('Submit'),
      '#button_type' => 'primary',
    );
   ...
   */
    ...
    /* $user = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
      ...
      if (($form_state->getValue('day5_walk_distance')) > $event_type) {
        $form_state->setErrorByName('day5_walk_distance', $this->t('Your distance is greater than the total event type.'));
      }*/
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    $image = $form_state->getValue('day5_image');
    $distanace = $form_state->getValue('day5_walk_distance');
    ...
    foreach ($nids as $nid) {
      $node = \Drupal\node\Entity\Node::load($nid); 
      $node->field_day5_pic->target_id =$image[0];
      $node->field_day5_distance->value =$distanace;
  ...
  $walker_full_name = ucfirst($walker_full_name);
     $last_pending_walk= $walker_total_distance-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist;
     $pending_walk =$walker_total_distance-$distanace-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist;
    $actual_pending_walk =$walker_total_distance-$distanace-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist;
    $current_walk_distance= $distanace+$walker_day1_dist+$walker_day2_dist+$walker_day3_dist+$walker_day4_dist;
     if($pending_walk < 0){
       $pending_walk = 0;
     }
...
if($walker_total_distance == $distanace || $pending_walk == 0 ){
  ...
  $certificate_html = getHtml5($walker_full_name, $walker_total_distance, $event_name, $days);
...
function getHtml5($walker_full_name, $walker_total_distance, $event_name, $days){
    $my_html='<HTML>
