...
/**
 * @file
 * Contains \Drupal\walk\Form\DayEightWalkForm.
 */
...
class DayEightWalkForm extends FormBase {
  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'day_eight_form';
  }
...
if(!empty($walker_image_url)){
       $form["step_8"] = array(
            ...
            '#prefix' => '<div class="edit-mode-8">',
           '#suffix' => '</div',
         );
        $form["step_8"]['day8_walk_distance'] = array (
      '#type' => 'textfield',
   '#attributes' => array(
      'min' => '0',
      'placeholder'=> 'Enter KMS',
    ...
    $form['#cache'] = ['max-age' => 0];
    $form['step_8']['actions']['#type'] = 'actions';
    $form['step_8']['actions']['submit'] = array(
      '#type' => 'submit',
      '#value' => $this->t('Submit'),
      '#button_type' => 'primary',
    );
    $form['walker_edit'] = array (
      ...
      '#prefix' =>'<div class="output-cont">',
       '#markup' => '<span id="toggle-step8">Edit</span>',
       '#suffix' =>'</div>'
    );
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    $image = $form_state->getValue('day8_image');
    $distanace = $form_state->getValue('day8_walk_distance');
    ...
    foreach ($nids as $nid) {
      ...
      $node->field_day8_pic->target_id =$image[0];
      $node->field_day8_distance->value =$distanace;
     ...
     $report_total_walk = $walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day2_dist+$walker_day9_dist+$walker_day10_dist;
   ...
   $database = \Drupal::database();
    ...
    $walker_full_name = ucfirst($walker_full_name);
     ...
     $current_walk_distance= $distanace+$walker_day1_dist+$walker_day2_dist+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day9_dist+$walker_day10_dist;

  $pending_walk =$walker_total_distance-$distanace-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist-$walker_day5_dist-$walker_day6_dist-$walker_day7_dist-$walker_day9_dist-$walker_day10_dist;
  ...
  $last_pending_walk= $walker_total_distance-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist-$walker_day5_dist-$walker_day6_dist-$walker_day7_dist-$walker_day8_dist-$walker_day9_dist-$walker_day10_dist;
    ...
    $overall_walk =$walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day2_dist+$walker_day9_dist+$walker_day10_dist;
...
if($walker_total_distance == $distanace || $pending_walk == 0 ){
  ...
  $certificate_html = getHtml8($walker_full_name, $walker_total_distance, $event_name, $days);
