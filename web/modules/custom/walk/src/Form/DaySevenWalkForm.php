...
/**
 * @file
 * Contains \Drupal\walk\Form\DaySevenWalkForm.
 */
...
class DaySevenWalkForm extends FormBase {
  ...
  $walker_image =$node->get('field_day7_pic')->getValue();
  if(!empty($walker_image[0])){
   ...
   $uri = file_create_url($image_uri);
    ...
    $walker_dist =$node->get('field_day7_distance')->getValue()[0]['value'];
  }
  else{
    $walker_image = 0;
  }
...
if(!empty($walker_image_url)){
  ...
  '#attributes' => array(
    ...
    $form['#cache'] = ['max-age' => 0];
    $form['step_7']['actions']['#type'] = 'actions';
    $form['step_7']['actions']['submit'] = array(
      '#type' => 'submit',
      '#value' => $this->t('Submit'),
      '#button_type' => 'primary',
    );
    $form['walker_edit'] = array (
      ...
      '#prefix' =>'<div class="output-cont">',
       '#markup' => '<span id="toggle-step7">Edit</span>',
       '#suffix' =>'</div>'
    );
   ...
   */
    public function validateForm(array &$form, FormStateInterface $form_state) {
     /*$user = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
      ...
      if (($form_state->getValue('day7_walk_distance')) > $event_type) {
        $form_state->setErrorByName('day7_walk_distance', $this->t('Your distance is greater than the total event type.'));
      }*/
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    $image = $form_state->getValue('day7_image');
    $distanace = $form_state->getValue('day7_walk_distance');
...
$report_total_walk = $walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day2_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
...
if($walker_total_distance == $distanace || $pending_walk == 0 ){
  ...
  $certificate_html = getHtml7($walker_full_name, $walker_total_distance, $event_name, $days);
