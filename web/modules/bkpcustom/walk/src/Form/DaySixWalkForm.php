...
/**
 * @file
 * Contains \Drupal\walk\Form\DaySixWalkForm.
 */
...
class DaySixWalkForm extends FormBase {
  /**
   * {@inheritdoc}
   */
  ...
  $walker_image =$node->get('field_day6_pic')->getValue();
  if(!empty($walker_image[0])){
    ...
    $walker_dist =$node->get('field_day6_distance')->getValue()[0]['value'];
  }
  else{
    $walker_image = 0;
  }
   ...
   if(!empty($walker_image_url)){
    ...
    $form['#cache'] = ['max-age' => 0];
    $form['step_6']['actions']['#type'] = 'actions';
    $form['step_6']['actions']['submit'] = array(
      '#type' => 'submit',
      '#value' => $this->t('Submit'),
      '#button_type' => 'primary',
    );
   ...
   */
    ...
    /* $user = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
      ...
      if (($form_state->getValue('day6_walk_distance')) > $event_type) {
        $form_state->setErrorByName('day6_walk_distance', $this->t('Your distance is greater than the total event type.'));
      }*/
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    $image = $form_state->getValue('day6_image');
    $distanace = $form_state->getValue('day6_walk_distance');
    ...
    foreach ($nids as $nid) {
      ...
      $node->field_day6_pic->target_id =$image[0];
      $node->field_day6_distance->value =$distanace;
	  ...
	  $report_total_walk = $walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day2_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
...
$current_walk_distance= $distanace+$walker_day1_dist+$walker_day2_dist+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;

  $pending_walk =$walker_total_distance-$distanace-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist-$walker_day5_dist-$walker_day7_dist-$walker_day8_dist-$walker_day9_dist-$walker_day10_dist;
     ...
     $last_pending_walk= $walker_total_distance-$walker_day1_dist-$walker_day2_dist-$walker_day3_dist-$walker_day4_dist-$walker_day5_dist-$walker_day6_dist-$walker_day7_dist-$walker_day8_dist-$walker_day9_dist-$walker_day10_dist;
      $overall_walk =$walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day2_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
...
if($walker_total_distance == $distanace || $pending_walk == 0 ){
  ...
  $certificate_html = getHtml6($walker_full_name, $walker_total_distance, $event_name, $days);
...
function getHtml6($walker_full_name, $walker_total_distance, $event_name, $days){
    $my_html='<HTML>
