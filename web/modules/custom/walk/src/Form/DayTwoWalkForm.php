...
/**
 * @file
 * Contains \Drupal\walk\Form\DayTwoWalkForm.
 */
...
class DayTwoWalkForm extends FormBase {
  /**
   * {@inheritdoc}
   */
  ...
  $walker_image =$node->get('field_day2_pic')->getValue();
  ...
  if(!empty($walker_image[0])){
   ...
   $uri = file_create_url($image_uri);
    ...
    $walker_dist =$node->get('field_day2_distance')->getValue()[0]['value'];
  }
  else{
    $walker_image = 0;
  }
    ...
    $form['day2_walk_distance'] = array (
      '#type' => 'textfield',
	 '#attributes' => array(
      'min' => '0',
      'placeholder'=> 'Enter KMS',
    ),
      '#title' => t('Day 2 | 23 October'),
      '#required' => TRUE,
    );
    $form['day2_image'] = [
        '#type' => 'managed_file',
        '#title' => t('Upload Day 2'),
...
if(!empty($walker_image_url)){
       $form["step_2"] = array(
            ...
            '#prefix' => '<div class="edit-mode-2">',
           '#suffix' => '</div',
         );
        $form["step_2"]['day2_walk_distance'] = array (
      '#type' => 'textfield',
    ...
    $form['#cache'] = ['max-age' => 0];
    $form['step_2']['actions']['#type'] = 'actions';
    $form['step_2']['actions']['submit'] = array(
      '#type' => 'submit',
      '#value' => $this->t('Submit'),
      '#button_type' => 'primary',
    );
    $form['walker_edit'] = array (
      ...
      '#prefix' =>'<div class="output-cont">',
       '#markup' => '<span id="toggle-step2">Edit</span>',
       '#suffix' =>'</div>'
    );
   ...
   */
    public function validateForm(array &$form, FormStateInterface $form_state) {
     /*$user = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
      ...
      if (($form_state->getValue('day2_walk_distance')) > $event_type) {
        $form_state->setErrorByName('day2_walk_distance', $this->t('Your distance is greater than the total event type.'));
      }
*/
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    $image = $form_state->getValue('day2_image');
    $distanace = $form_state->getValue('day2_walk_distance');
    ...
    foreach ($nids as $nid) {
      ...
      $node->field_day2_pic->target_id =$image[0];
      $node->field_day2_distance->value =$distanace;
      ...
      $report_total_walk = $walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
    ...
    $walker_full_name = ucfirst($walker_full_name);
     ...
     $current_walk_distance= $distanace+$walker_day1_dist+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
 ...
 $overall_walk =$walker_day1_dist+$distanace+$walker_day3_dist+$walker_day4_dist+$walker_day5_dist+$walker_day6_dist+$walker_day7_dist+$walker_day8_dist+$walker_day9_dist+$walker_day10_dist;
...
if($walker_total_distance == $distanace || $pending_walk == 0 ){
  ...
  $certificate_html = getHtml2($walker_full_name, $walker_total_distance, $event_name, $days);
...
function getHtml2($walker_full_name, $walker_total_distance, $event_name, $days){
    $my_html='<HTML>
